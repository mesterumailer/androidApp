...
class SettingsActivity : Activity() {
    ...
    private val danger = Color.rgb(188, 73, 73)
    ...
    private fun createCategoryCard(rule: FilterRule, index: Int): View = LinearLayout(this).apply {
        ...
        actions.addView(TextView(this@SettingsActivity).apply {
            text = "ویرایش"
            textSize = 11f
            gravity = Gravity.CENTER
            setTextColor(accent)
            background = roundedBackground(Color.rgb(239, 243, 255), 11)
        ...
        actions.addView(TextView(this@SettingsActivity).apply {
            ...
            contentDescription = "حذف ${rule.displayName}"
            setOnClickListener { confirmDelete(rule) }
        })
        addView(actions, LinearLayout.LayoutParams(dp(62), -2))
    }.apply {
    ...
    private fun multiEditor(title: String, values: List<String>): Pair<TextView, EditText> =
        label("$title (هر مورد در یک خط)") to EditText(this).apply {
    ...
    private fun numberEditor(title: String, value: Int): Pair<TextView, EditText> =
        label(title) to EditText(this).apply {
